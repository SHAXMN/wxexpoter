...
// Scan all document attachments from iTunes backup
bool ITunesDb::scanDocumentAttachments(std::vector<AttachmentInfo>& attachments) const
{
    ...
    {
        if (file == nullptr || file->relativePath.empty())
        {
            continue;
        }

        const std::string& path = file->relativePath;

        // Check if file is in OpenData directory
        if (path.find("/OpenData/") == std::string::npos)
        {
            continue;
        }
        ...
        // Extract session hash from path
        // Format: Documents/.../OpenData/{hash}/{msgId}.ext
        size_t openDataPos = path.find("/OpenData/");
        if (openDataPos != std::string::npos)
        {
            size_t hashStart = openDataPos + 10; // "/OpenData/" length
            size_t hashEnd = path.find('/', hashStart);
            if (hashEnd != std::string::npos)
            {
                info.sessionHash = path.substr(hashStart, hashEnd - hashStart);
            }
        }
...
int ITunesDb::copyAttachments(const std::vector<AttachmentInfo>& attachments, const std::string& outputPath) const
{
    ...
    {
        if (att.sessionHash.empty() || att.filename.empty())
        {
            continue;
        }

        // Build destination directory: {outputPath}/attachments/{sessionHash}/
        std::string destDir = combinePath(outputPath, "attachments");
        destDir = combinePath(destDir, att.sessionHash);
        ...
        // Copy file using existing copyFile method
        if (copyFile(att.relativePath, destDir, att.filename, false))
        {
            copied++;
        }
    }
