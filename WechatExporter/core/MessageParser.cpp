...
bool MessageParser::parseAppMsg(const WXMSG& msg, const Session& session, std::string& senderId, std::string& forwardedMsg, std::string& forwardedMsgTitle, TemplateValues& tv) const
{
    ...
    {
        // Failed to parse APPMSG type (possibly compressed message)
        // Try to detect and link document attachments
...
bool MessageParser::parseAppMsgAttachment(const WXAPPMSG& appMsg, const XmlParser& xmlParser, const Session& session, TemplateValues& tv) const
{
    ...
    // If XML parsing failed, try to detect extension from OpenData
    if (attachFileExtName.empty())
    {
        ...
        {
            // Remove leading dot
            if (attachFileExtName[0] == '.')
            {
                attachFileExtName = attachFileExtName.substr(1);
            }
        }
    }
    ...
    // Build file paths
    std::string attachFileName = m_userBase + "/OpenData/" + session.getHash() + "/" + appMsg.msg->msgId + "." + attachFileExtName;
    std::string attachOutputFileName = appMsg.msg->msgId + "." + attachFileExtName;

    // Copy to attachments directory instead of assets
    ...
    {
        // Get file size
        const ITunesFile* file = m_iTunesDb.findITunesFile(attachFileName);
        std::string fileSize;
        if (file != nullptr)
        {
        ...
        // Use title if available, otherwise use file type label
        if (!title.empty())
        {
            tv["%%SHARINGTITLE%%"] = title + fileSize;
        }
        else
        {
            tv["%%SHARINGTITLE%%"] = getFileTypeLabel("." + attachFileExtName) + fileSize;
        }
...
// Try to parse compressed attachment message
bool MessageParser::tryParseCompressedAttachment(const WXMSG& msg, const Session& session, TemplateValues& tv) const
{
    ...
    // Try to detect file extension from OpenData directory
    std::string ext = detectExtensionFromOpenData(msgId, sessionHash);
    ...
    if (m_iTunesDb.copyFile(vpath, destDir, filename))
    {
        ...
        tv["%%SHARINGTITLE%%"] = getFileTypeLabel(ext) + fileSize;
        tv["%%MESSAGE%%"] = "";
        tv["%%MSGTYPE%%"] = "file";
...
// Detect file extension from OpenData directory
std::string MessageParser::detectExtensionFromOpenData(const std::string& msgId, const std::string& sessionHash) const
{
    ...
    for (const char* ext : extensions)
    {
        ...
        // Check if file exists in iTunes backup
        const ITunesFile* file = m_iTunesDb.findITunesFile(vpath);
        if (file != nullptr)
        {
            return ext;
...
std::string MessageParser::getFileTypeLabel(const std::string& ext) const
{
    if (ext == ".pdf")
    {
        return m_resManager.getLocaleString("PDF Document");
    }
    else if (ext == ".doc" || ext == ".docx")
    {
        return m_resManager.getLocaleString("Word Document");
    }
    else if (ext == ".xls" || ext == ".xlsx")
    {
        return m_resManager.getLocaleString("Excel Spreadsheet");
    }
    else if (ext == ".ppt" || ext == ".pptx")
    {
        return m_resManager.getLocaleString("PowerPoint Presentation");
    }
    else if (ext == ".zip" || ext == ".rar" || ext == ".7z")
    {
        return m_resManager.getLocaleString("Archive");
    }
...
std::string MessageParser::formatFileSize(size_t bytes) const
{
